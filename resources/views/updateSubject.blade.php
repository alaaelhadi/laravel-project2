...
<div class="container">
  ...
  <form action="{{ route('updatesubject',$subject->id) }}" method="post" enctype="multipart/form-data">
    ...
    <div class="form-group">
      ...
      <input type="number" class="form-control" id="title" placeholder="Enter start time" name="start_time" value="{{ $subject->start_time }}">
      @error('start_time')
      <div class='alert alert-warning'>
        {{ $message }}
      </div>
    ...
    <div class="form-group">
      ...
      <input type="number" class="form-control" id="title" placeholder="Enter end time" name="end_time" value="{{ $subject->end_time }}">
      @error('end_time')
      <div class='alert alert-warning'>
        {{ $message }}
      </div>
    ...
    <div class="form-group">
        <select name="teacher_id">
            ...
            <option value="{{ $teacher->id }}" @selected($teacher->id == $subject->teasher_id)>{{ $teacher->name }}</option>
            @endforeach
        </select>
        @error('teacher_id')
            <div class='alert alert-warning'>
